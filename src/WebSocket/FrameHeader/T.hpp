    ...
    masking_key ({0})
{
	try
	{
		...
		// It's possible that we could represent a false 'use_mask' by just
		// leaving the masking key at 0.
		...
		if (this->use_mask)
		{
			decltype (this->masking_key)::size_type i;
			for (i = 0; i < 4; ++i)
				this->masking_key[i] = (uint8_t) input_stream.get ();
		}
	}
