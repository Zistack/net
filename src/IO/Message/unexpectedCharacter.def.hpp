...
unexpectedCharacter (char c)
{
	...
	if (Class::control (c)) message += Util::toHex (c);
	else message += c;
...
unexpectedCharacter (char c, char e)
{
	...
	if (Class::control (e)) expected = Util::toHex (e);
	else expected = {e};
...
unexpectedCharacter (char16_t c, char16_t e)
{
	...
	else expected += Util::runeToUTF8CodePoint ((char32_t) e);
...
unexpectedCharacter (char32_t c, char32_t e)
{
	...
	else expected += Util::runeToUTF8CodePoint (e);
