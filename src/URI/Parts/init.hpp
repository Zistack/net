...
T::init (InputStream && input_stream)
{
	...
	if (IO::Util::test (std::forward<InputStream> (input_stream), ':'))
	{
		...
		if (IO::Util::test (std::forward<InputStream> (input_stream), '/'))
		{
			...
			if (IO::Util::test (std::forward<InputStream> (input_stream), '/'))
			{
				input_stream.get ();
				this->authority =
				    getAuthorityPart (std::forward<InputStream> (input_stream));
			}
			else
			{
				this->path.push_back ('/');
	...
	{
		input_stream.get ();
		this->query = getQueryPart (std::forward<InputStream> (input_stream));
	}
	...
	if (IO::Util::test (std::forward<InputStream> (input_stream), '#'))
	{
		input_stream.get ();
		this->fragment =
		    getFragmentPart (std::forward<InputStream> (input_stream));
	}
}
